...
def main():
    ...
    disp_net = models.DispResNet(18, True).to(device)
    ...
    intrinsic = torch.from_numpy(np.array([[[479.8531, 0, 408.4586],
                           [0, 489.2307, 126.6926],
    ...
    for iter in tqdm(range(n - 1)):
        ...
        tgt_depth = [1/disp.unsqueeze(0) for disp in disp_net(tensor_img1)]
        
        pose, _, _ = pose_net(tensor_img1, tensor_img2, tgt_depth, intrinsic)

        pose_mat = pose_vec2mat(pose[0]).squeeze(0).cpu().detach().numpy()
    ...
    filename = os.path.join(args.output_dir, args.sequence + ".txt")
